...
def main() -> None:
    ...
    # Prevent huge scans by default
    if args.blocks > 5000:
        print("⚠️  Limiting --blocks to 5000 to avoid excessive RPC load.", file=sys.stderr)
        args.blocks = 5000
    ...
    print(
        f"💵 Effective Price:   p50={ep['p50']}  p95={ep['p95']}  min={ep['min']}  "
        f"max={ep['max']}  (n={ep['count']})"
    )
    print(
